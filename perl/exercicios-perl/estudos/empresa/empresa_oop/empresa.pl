...
do {
	system 'clear';
	say "EMPREGADOS CADASTRADOS: ".Empregado->cadastrados();
...
sub inserirFuncionario() {
	...
	do {
		...
		chomp($dados = <STDIN>);
	 	if ($dados =~ /^(\s*\D+) - (\d+\D*) - (\d{1,2})\/(\d{1,2})\/(\d{4}) - (\D+)$/) {
...
sub aumentarSalarioDeptFinancas {
	$_->aumentarSalario(200) for grep { lc $_->dept() eq 'financas' } @empregados;
}
